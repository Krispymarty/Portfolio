...
function CameraRig({ chapter, reducedMotion }: { chapter: number; reducedMotion: boolean }) {
  ...
  useFrame((_, delta) => {
    ...
    const pointerX = reducedMotion || mobile ? 0 : pointer.x * 0.14;
    const pointerY = reducedMotion || mobile ? 0 : pointer.y * 0.08;
    const target = new THREE.Vector3(
      (mobile ? 6.9 : 6.5) + chapterDepth + pointerX,
      (mobile ? 4.7 : 4.25) + pointerY,
      mobile ? 8.5 : 7.6,
    );
...
function WorkspaceModel({ chapter, reducedMotion, onModelReady }: WorkspaceSceneProps) {
  ...
  useFrame((_, delta) => {
    ...
    const mobile = size.width < 760;
    const targetScale = mobile ? 0.67 : size.width < 1100 ? 0.82 : 0.94;
    const targetX = mobile ? 0.05 : 1.38;
    const targetY = mobile ? -0.18 : -0.02;
    const targetRotation =
      -0.08 + chapter * 0.018 + (reducedMotion || mobile ? 0 : pointer.x * 0.025);
...
export default function WorkspaceScene(props: WorkspaceSceneProps) {
  return (
    <Canvas
      ...
      performance={{ min: 0.5 }}
    >
      <PerspectiveCamera makeDefault position={[6.5, 4.25, 7.6]} fov={34} near={0.1} far={60} />
