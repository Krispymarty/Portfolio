...
export function BuildLogSection({ buildLogs }: { buildLogs: LogEntry[] }) {
    ...
    return (
      <section id="build-log" className="scroll-mt-24 py-16">
        <div className="flex items-center gap-4 mb-12">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-text-primary">Engineering Log</h2>
  ...
  return (
    <section id="build-log" className="scroll-mt-24 py-16">
      ...
      <p className="text-text-secondary mb-12 max-w-xl">Real-time documentation of challenges, solutions, and lessons learned while building.</p>
            ...
            <div className="flex flex-col md:flex-row gap-6 md:gap-10">
              ...
              <div className="flex-1 bg-surface border border-border rounded-2xl p-6 md:p-8 hover:border-accent/30 transition-colors">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="text-xs font-bold text-red-400 uppercase tracking-widest mb-2">Challenge</h4>
                    <p className="text-text-primary/90 text-sm leading-relaxed">{log.challenge}</p>
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-green-400 uppercase tracking-widest mb-2">Solution</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 pt-6 border-t border-border/50">
                  <div className="bg-background rounded-xl p-4 border border-border">
                    <h4 className="text-xs font-bold text-accent uppercase tracking-widest mb-2">Lesson Learned</h4>
                    <p className="text-text-secondary text-sm leading-relaxed italic">{log.lessonLearned}</p>
                  </div>
                  <div className="bg-background rounded-xl p-4 border border-border">
                    <h4 className="text-xs font-bold text-primary uppercase tracking-widest mb-2">Next Step</h4>
                    <p className="text-primary text-sm font-medium">{log.nextStep}</p>
                  </div>
                </div>
              </div>
            </div>
